...
def evaluate_twoclass(predictions, truth):
...
def main():
    ...
    if options.model_type == 'twoclass':
        ...
        total = 0
        count = 0
        for fn,lab in zip(filelist_valid,labels):
            print(fn,len(lab))
            t,results = evaluate_twoclass(np.argmax(preds[fn],1),lab)
            total+=t
            count+=(results['accuracy']*t)
        print(count/total)
        labels = load_labels(filelist_test,preds,fps,datadir)
        print(len(labels))
        total = 0
        count = 0
